...
const hasAllEqual = (arr) => arr.every((val) => val === arr[0]);
const hasUniqueOnly = (arr) => Array.from(new Set(arr)).length === arr.length;
...
describe('Novu Node.js package - Retries and idempotency-key', () => {
    ...
    const novu = new index_1.Novu('fake-key', {
    ...
    test.each(NON_RECOVERABLE_ERRORS)('should not retry on non-recoverable %i error', async (status, message) => {
        (0, nock_1.default)(BACKEND_URL).get(TOPICS_PATH).times(3).reply(status, { message });
        (0, nock_1.default)(BACKEND_URL).get(TOPICS_PATH).reply(200, [{}, {}]);
        await expect(novu.topics.list({})).rejects.toMatchObject({
            response: { status },
        });
    ...
    it('should retry on various errors until it reach successful response', async () => {
        ...
        (0, nock_1.default)(BACKEND_URL).get(TOPICS_PATH).reply(200, [{}, {}]);
        const novuClient = new index_1.Novu('fake-key', {
            backendUrl: BACKEND_URL,
            retryConfig: {
                initialDelay: 0,
                waitMin: 0.2,
    ...
    describe('defaultRetryCondition function', () => {
        test.each(NON_RECOVERABLE_ERRORS)('should return false when HTTP status is %i', (status) => {
            const err = new HttpError(status);
            expect((0, index_1.defaultRetryCondition)(err)).toEqual(false);
        });
        test.each(retry_1.RETRYABLE_HTTP_CODES)('should return true when HTTP status is %i', (status) => {
            const err = new HttpError(status);
