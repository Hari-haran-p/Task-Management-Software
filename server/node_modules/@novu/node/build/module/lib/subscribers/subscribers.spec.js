...
describe('test use of novus node package - Subscribers class', () => {
    ...
    test('should mark message action as seen', async () => {
        mockedAxios.post.mockResolvedValue({});
        await novu.subscribers.markMessageActionSeen('test-action-type-sub', 'message-123', shared_1.ButtonTypeEnum.PRIMARY, { status: shared_1.MessageActionStatusEnum.DONE });
        expect(mockedAxios.post).toHaveBeenCalled();
    ...
    test('should mark all subscriber messages as read', async () => {
        mockedAxios.post.mockResolvedValue({});
        await novu.subscribers.markAllMessagesAs('test-action-type-sub', shared_1.MessagesStatusEnum.READ);
        expect(mockedAxios.post).toHaveBeenCalled();
    ...
    test('should mark all subscriber messages as read for feed', async () => {
        mockedAxios.post.mockResolvedValue({});
        await novu.subscribers.markAllMessagesAs('test-action-type-sub', shared_1.MessagesStatusEnum.READ, 'feed-123');
        expect(mockedAxios.post).toHaveBeenCalled();
        expect(mockedAxios.post).toHaveBeenCalledWith('/subscribers/test-action-type-sub/messages/mark-all', {
            markAs: shared_1.MessagesStatusEnum.READ,
