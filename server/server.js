...
app.post('/api/addNewTask', async (req, res) => {
  const { task_name, priority, due_date, status_id, assigned_by, task_desc, selectedUsers } = req.body; // Get selectedUsers

  const taskQuery = 'INSERT INTO tasks (title, priority, due_date, column_id, assigned_by, task_desc) VALUES(?,?,?,?,?,?)';
  ...
  try {
    ...
    const taskResult = await db.query(taskQuery, [task_name, priority, due_date, status_id, assigned_by, task_desc]);
    ...
    res.json({ result: "New task added and users assigned" });
  } catch (err) {
...
app.put('/api/updateTaskData', async (req, res) => {
  ...
  try {
    ...
    const updateResult = await db.query(
      ...
      [task_name, priority, due_date, status_id, assigned_by_id, task_desc, task_id]
    );
